...
import { getByPath, fileKindLabel } from '../fs/vfs';
    ...
    <div className="flex flex-col h-full text-white" style={{ background: 'rgba(18,19,23,0.92)' }}>
      <div className="flex items-center gap-1 h-[40px] px-3 shrink-0" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.08)' }}>
        ...
        <div className="flex items-center gap-0.5 ml-1">
          <MacToolbarButton icon="minus.magnifyingglass" size={26} onClick={() => setZoom(z => Math.max(0.25, z - 0.15))} />
          <MacToolbarButton icon="plus.magnifyingglass" size={26} onClick={() => setZoom(z => Math.min(4, z + 0.15))} />
        </div>
      ...
      <div className="flex-1 flex items-center justify-center overflow-auto" style={{ background: 'rgba(0,0,0,0.45)' }}>
        ...
        {isText && <TextView node={node} />}
        {isPdf && <PdfView node={node} />}
...
function TextView({ node }) {
  ...
  return (
    <div className="max-w-[760px] w-full mx-auto px-8 py-10 overflow-y-auto h-full">
...
function PdfView({ node }) {
  return (
    <iframe src={node.contentUrl} title={node.name} className="w-full h-full border-0 bg-white" />
  );
}
