"""Divergence Meter (Steins;Gate) - Import + Discovery
Dark moody sci-fi, warm amber nixie glow.
"""
import bpy, math
...
bpy.ops.import_scene.gltf(filepath=f'{ASSETS}/divergence_meter_steinsgate.glb')
...
for obj in bpy.data.objects:
    ...
    if obj.children:
        info += f' | Children({len(obj.children)}): {[c.name for c in obj.children[:5]]}'
    print(info)
...
for m in bpy.data.materials:
    print(f'  {m.name}')
print('Divergence Meter discovery complete')
